...
4. [ ] Construct a cDBG from the fasta file in #3 with k=25.
    ...
    - [ ] 3rd scenario: component length > 75nt and when searching in the Fasta file, we will find that **only two unitigs** aligned to a component, these two edges(unitigs) from two separate components should be merged to one component.
    - [ ] 4th scenario: Matched multiple unitigs in different components,  